...
assert.match(pageHtml, /styles\.css\?v=2\.11\.1/);
assert.match(pageHtml, /id="dialog-previous"/);
assert.match(pageHtml, /id="dialog-next"/);
assert.match(pageHtml, /id="dialog-position"/);
assert.match(pageHtml, /data-result-filter="repeat_detection"/);
assert.match(pageHtml, /id="result-count-repeat"/);
...
assert.match(pageHtml, /value="repeat_detection">疑似背景誤判/);
assert.match(pageHtml, /<option value="empty">空觸發<\/option>/);
assert.match(pageHtml, /<option value="animal">有動物<\/option>/);
assert.match(pageHtml, /<option value="person_vehicle">人／車<\/option>/);
assert.doesNotMatch(pageHtml, /<option value="ai_complete">/);
...
assert.match(appSource, /function aiEventContentLabels\(event\)/);
assert.match(appSource, /"AIRepeatDetection", "AIRepeatDetectionSupport"/);
assert.match(appSource, /function syncEventsFromServer/);
assert.match(appSource, /BroadcastChannel/);
assert.match(appSource, /\["人工最終結果", humanResultLabel\(event\)\]/);
assert.match(appSource, /function handleImageDialogKeydown\(event\)/);
assert.match(appSource, /event\.key === "ArrowLeft"/);
assert.match(appSource, /event\.key === "ArrowRight"/);
assert.match(appSource, /function createVideoPlayer\(event\)/);
assert.match(appSource, /AIRepeatDetection === "yes"/);
assert.match(appSource, /String\(event\.AIEventLabels \|\| ""\)/);
assert.match(appSource, /contentFilter === "person_vehicle"/);
assert.match(appSource, /function syncModalScrollLock\(\)/);
assert.match(appSource, /confirm: "RESET_AI_BATCH", deploymentId/);
assert.match(appSource, /confirm: "CLEAR_UPLOAD_BATCH", deploymentId/);
checks.push("ui:batch-separated-result-and-review-logic");

const stylesResponse = await fetch(`${baseUrl}/styles.css?v=2.11.1`);
...
assert.match(stylesSource, /\.image-dialog \{ box-sizing: border-box;/);
assert.match(stylesSource, /\.image-dialog\[open\] \{ display: grid; grid-template-rows: minmax\(0, 1fr\) auto;/);
assert.match(stylesSource, /\.image-dialog img \{[^}]*width: 100%;[^}]*height: 100%;[^}]*object-fit: contain;/s);
assert.match(stylesSource, /\.photo-frame \{[^}]*display: grid;[^}]*grid-template-rows: auto auto;/s);
assert.match(stylesSource, /\.photo-label \{ position: static;[^}]*border-top:/s);
...
assert.ok(["READY", "MODEL_MISSING", "NOT_INSTALLED", "BROKEN"].includes(aiStatus.body.runtime?.status));
assert.equal(typeof aiStatus.body.runtime?.detectorModelReady, "boolean");
...
assert.equal(typeof aiBatch.body.status.repeatCandidates, "number");
...
const webEvents = eventPayload.body.events.filter((event) => event.SourceType === "web_upload");
const webBatch = webEvents[0]?.DeploymentID || "";
if (webBatch) {
  const scopedAiBatch = await json(`/api/ai/batch?identifySpecies=0&mode=fast&deploymentId=${encodeURIComponent(webBatch)}`);
  assert.equal(scopedAiBatch.response.status, 200);
  assert.equal(scopedAiBatch.body.status.deploymentId, webBatch);
  assert.equal(scopedAiBatch.body.status.total, webEvents.filter((event) => event.DeploymentID === webBatch).length);
  checks.push("ai:batch-status-scoped-to-import");
}
...
assert.equal(typeof first.AIRepeatDetection, "string");
assert.match(first.videoPreview, /^\/video-preview\//);
