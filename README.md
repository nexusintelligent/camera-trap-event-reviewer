...
1. 下載或解壓專案資料夾。
2. 第一次使用時，雙擊 `安裝AI辨識環境.cmd`。它會在使用者資料夾建立專用 Python 3.11 環境，安裝 MegaDetector、SpeciesNet 並下載 MegaDetector 權重；第一次需要網路與數分鐘時間。
...
- 啟動程式需要 Node.js 20 以上版本；AI 安裝程式會自行建立或重用 Python 3.11 環境。
- 若服務未啟動，查看 `logs\server.stderr.log`。
- 若模型遺失或損壞，重新執行 `安裝AI辨識環境.cmd`。
